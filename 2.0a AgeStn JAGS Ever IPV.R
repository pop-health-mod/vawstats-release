...
data(vwa_data) # data not included... not yet publicly released

# Rename.
Data <- vwa_data; dim(Data)

# ---- Ever IPV ----
IPV_ <- subset(Data, (parametertype == "Ever IPV" | parametertype == "SexVio Ever") & #
                      viotime == "Ever" &
...
length(unique(IPV_$ID))
length(unique(IPV_$iso3))
summary(IPV_)
...
IPV_ <- na.omit(subset(IPV_, select = c(ID, country, region, SuperRegion, iso3, startyr, agerange, Time,
                                        ...
                                        parametertype, pstat, currpart, viotime, violence, severe, loqual,
                                        spouseonly, currpart, geo, notviostudy1, nointrain, imputation_denom))); dim(IPV_)               
any(is.na(IPV_))
...
IPV_$RowID <- seq(1:nrow(IPV_))
dim(IPV_)[1]; length(unique(IPV_$ID)); length(unique(IPV_$IDage))
sum(IPV_$denom_imp)

IPV <- optimal_set(IPV_, outcome = "Ever IPV")
dim(IPV)[1]; length(unique(IPV$ID)); length(unique(IPV$IDage))
IPV1 <- IPV

# We remove larger age-group if age-disaggregated is available
IPV <- remove_larger_agegrp(IPV)
dim(IPV)[1]; length(unique(IPV$ID))
...
IPV <- IPV[order(IPV$region, IPV$iso3, IPV$startyr, IPV$ID, IPV$violence, 
                 IPV$severe, IPV$pstat, IPV$geo, IPV$AgeM), ]
IPV$RowID <- seq(1, dim(IPV)[1], 1)

# If we only include "Sex and/or Physical IPV",
# we reduce the number of studies from 277 to 196,
# the number of countries from 131 to 118,
# the number of regions from 21 to 20,
# and the number of women from 4,222,024 to 1,683,683.
nrow(IPV)
...
write.csv(age_dist, file = "VAW age distribution - Ever IPV.csv")

# ---- Map of data availability ----
world_map_data(study_id = IPV$ID, iso3 = IPV$iso3, 
               outcome = "Ever IPV", suffix = "2000-2018")
...
# ECU 2011 is OK b/c rural urban only.
...
# country-years
length(unique(paste(IPV$iso3, IPV$Time)))
...
sdat_ever <- SDat_original
sdat_ever$sup <- IPV$SuperRegion
sdat_ever$cnt <- IPV$iso3
sdat_ever$reg <- IPV$region
sdat_ever$nat <- IPV$National
sdat_ever$IPV <- IPV
saveRDS(sdat_ever, file = "sdat_ever.rds")
